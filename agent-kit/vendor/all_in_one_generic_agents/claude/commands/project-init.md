---
description: Detect a project's stack and produce a dry-run all_in_one_generic_agents onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable all_in_one_generic_agents onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
3. Use all_in_one_generic_agents's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- all_in_one_generic_agents config: `ecc-install.json`
- optional stack map: `config/project-stack-mappings.json` in the all_in_one_generic_agents repo

When the all_in_one_generic_agents checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed all_in_one_generic_agents manifests and explicit user choices.
...
3. Resolve the smallest useful all_in_one_generic_agents plan:
...
- `/generic-agents-guide` for interactive feature discovery before installing
