...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value all_in_one_generic_agents-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
metadata:
...
This is the all_in_one_generic_agents-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against all_in_one_generic_agents-native coverage
...
- Prefer all_in_one_generic_agents-native workflows over generic "install another plugin" advice when all_in_one_generic_agents can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in all_in_one_generic_agents
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent all_in_one_generic_agents workspaces
...
4. all_in_one_generic_agents surface
...
- existing all_in_one_generic_agents skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but all_in_one_generic_agents lacks a billing-operator skill"
- "Google Drive is connected, but there is no all_in_one_generic_agents-native Google Workspace operator workflow"
...
2. whether all_in_one_generic_agents already has parity
3. whether all_in_one_generic_agents only has primitives
4. whether all_in_one_generic_agents is missing the workflow entirely

### Phase 3: Turn Gaps Into all_in_one_generic_agents Decisions

For every real gap, recommend the correct all_in_one_generic_agents-native shape:

| Gap Type | Preferred all_in_one_generic_agents Shape |
|----------|---------------------|
...
2. **Parity**
   - where all_in_one_generic_agents already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but all_in_one_generic_agents lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete all_in_one_generic_agents-native additions, ordered by impact
...
- If all_in_one_generic_agents already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what all_in_one_generic_agents should own next.
