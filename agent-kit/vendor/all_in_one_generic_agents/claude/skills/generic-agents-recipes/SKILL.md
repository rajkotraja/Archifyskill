...
- `generic-agents-guide` — lists commands and where to read docs, but as a flat catalog.
...
- User wants deep docs for ONE command — use `generic-agents-guide`.
...
5. **Where to read** — the `commands/<name>.md` path plus `/generic-agents-guide <name>`.
...
Read full docs:
  commands/<cmd1>.md   (or: /generic-agents-guide <cmd1>)
```

## Examples

**Catalog:** `/generic-agents-recipes` → prints the family table and total count.

**Match:** `/generic-agents-recipes plan a whole app upfront then auto-build with adversarial
...
**Match:** `/generic-agents-recipes fix a bug in my Go service` → Best fit: `orch-fix-defect`
...
- Not per-command deep docs — that's `generic-agents-guide`.
