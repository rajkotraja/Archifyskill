...
This guide helps you migrate from Claude Code to OpenCode while using the all_in_one_generic_agents configuration.
...
### all_in_one_generic_agents Plugin Hooks Included

The all_in_one_generic_agents OpenCode configuration includes translated hooks:
...
### 2. Use the all_in_one_generic_agents OpenCode Configuration
...
│   ├── ecc-hooks.ts           # All all_in_one_generic_agents hooks as plugins
...
### Option 1: Use all_in_one_generic_agents Configuration Directly
...
This only loads the published all_in_one_generic_agents OpenCode plugin module (hooks/events and exported plugin tools).
It does **not** automatically inject all_in_one_generic_agents's full `agent`, `command`, or `instructions` config into your project.

If you want the full all_in_one_generic_agents OpenCode workflow surface, use the repository's bundled `.opencode/opencode.json` as your base config or copy these pieces into your project:
- `.opencode/commands/`
- `.opencode/prompts/`
...
4. If you installed only `plugin: ["ecc-universal"]`, note that npm plugin install does not auto-add all_in_one_generic_agents commands or agents to your project config
...
- **all_in_one_generic_agents Configuration**: Report to <url>
