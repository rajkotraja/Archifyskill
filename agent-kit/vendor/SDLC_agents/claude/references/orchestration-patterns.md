...
```
/review → sdlc-code-reviewer (with code-review-and-quality skill) → report
```
...
```
user runs:  /spec  →  /sdlc-plan  →  /build  →  /test  →  /review  →  /ship
```
...
Plugin subagents go in `agents/` at the plugin root. This repo is a plugin (`.claude-plugin/plugin.json`), so `agents/sdlc-code-reviewer.md`, `agents/security-auditor.md`, and `agents/test-engineer.md` are auto-discovered when the plugin is enabled. No path configuration needed.
...
Don't redefine these. Layer your specialist personas (sdlc-code-reviewer, security-auditor, test-engineer) on top of them.
...
three teammates using the existing agent types:
  ...
  - sdlc-code-reviewer  — investigate race conditions and blocking calls
...
4. When `sdlc-code-reviewer` finds a `Promise.all` that should be sequential, it messages `security-auditor` to confirm the auth call isn't part of the race. `security-auditor` checks and replies — either confirming the race is the real issue or producing counter-evidence.
...
```
/work → router-persona → "this needs a review" → sdlc-code-reviewer → router (paraphrases) → user
```
...
A `sdlc-code-reviewer` that internally invokes `security-auditor` when it sees auth code.
...
An agent that calls `/spec`, then `/sdlc-plan`, then `/build`, etc. on the user's behalf.
...
`/ship` calls a `pre-ship-coordinator` that calls a `quality-coordinator` that calls `sdlc-code-reviewer`.
