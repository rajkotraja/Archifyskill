...
```
/review → addy-code-reviewer (with code-review-and-quality skill) → report
```
...
```
user runs:  /spec  →  /addy-plan  →  /build  →  /test  →  /review  →  /ship
```
...
Plugin subagents go in `agents/` at the plugin root. This repo is a plugin (`.claude-plugin/plugin.json`), so `agents/addy-code-reviewer.md`, `agents/security-auditor.md`, and `agents/test-engineer.md` are auto-discovered when the plugin is enabled. No path configuration needed.
...
Don't redefine these. Layer your specialist personas (addy-code-reviewer, security-auditor, test-engineer) on top of them.
...
4. When `addy-code-reviewer` finds a `Promise.all` that should be sequential, it messages `security-auditor` to confirm the auth call isn't part of the race. `security-auditor` checks and replies — either confirming the race is the real issue or producing counter-evidence.
...
A `addy-code-reviewer` that internally invokes `security-auditor` when it sees auth code.
...
An agent that calls `/spec`, then `/addy-plan`, then `/build`, etc. on the user's behalf.
